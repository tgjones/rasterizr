	...
	public struct TriangleGradients
	{
		...
		public TriangleGradients(ScreenSpaceVertex[] vertices)
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				InterpolatedVertexAttributes[i].Attributes = new InterpolatedVertexAttribute[vertices[i].Attributes.Count];
				for (int j = 0; j < vertices[i].Attributes.Count; ++j)
				{
					InterpolatedVertexAttributes[i].Attributes[j] = new InterpolatedVertexAttribute
					{
						Name = vertices[i].Attributes[j].Name,
						Semantic = vertices[i].Attributes[j].Semantic,
			...
			VertexAttributeStepValues = new VertexAttributeStepValues[vertices[0].Attributes.Count];
			for (int i = 0; i < vertices[0].Attributes.Count; ++i)
			{
