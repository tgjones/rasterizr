...
{
	public class TextureMipMappingScene : PresetSceneBase
	{
		public override void Draw(RasterizrDevice device)
		{
			device.ClearDepthBuffer(1);
			device.ClearRenderTarget(new ColorF(0.8f, 0.8f, 0.8f));
			...
			device.InputAssembler.Vertices = new[]
			{
				...
				new VertexPositionColorTexture(new Point3D(-1, 0, -10), ColorsF.White, new Point2D(1, 0)),
				new VertexPositionColorTexture(new Point3D(1, 0, -10), ColorsF.White, new Point2D(1, 1))
			};
